          ...
          <tbody>
            {teacherData.map((row, idx) => (
              <tr key={idx} className="h-8">
                ...
                <td onClick={()=>setDay(row.day)} className={day === row.day? " border border-black text-white bg-green-100 font-bold text-center": "border border-black bg-gray-50 font-bold text-center"}>{row.day}</td>
              </tr>
            ))}
          </tbody>
        </table>
